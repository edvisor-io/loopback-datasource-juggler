 ...
 */
RelationDefinition.prototype.applyScope = function(modelInstance, filter, options, cb) {
  filter = filter || {};
  ...
  if (typeof this.scope === 'function') {
    ...
    if (this.scope.length === 4) {
      waiting = true;
      scope = this.scope.call(this, modelInstance, filter, options, function(scope){
        mergeQuery(filter, scope);
        if (cb) {
          return cb();
        }
      });
    } else if (this.scope.length === 3) {
      waiting = true;
