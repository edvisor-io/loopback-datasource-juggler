 ...
 * Custom validator
 */
function validateCustom(attr, conf, err, context, done) {
  if (conf.customValidator.length === 3) {
    conf.customValidator.call(this, err, context, done);
  } else {
    if (done === undefined) {
      done = context;
    }
 ...
 */
function validateUniqueness(attr, conf, err, context, done) {
  if (done === undefined) {
    done = context;
    context = undefined;
  }
...
function validationFailed(inst, attr, conf, cb) {
  ...
  if (conf.remoteCtx) {
    validatorArguments.push(conf.remoteCtx);
  }
  if (cb) {
