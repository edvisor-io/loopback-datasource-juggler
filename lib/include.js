...
Inclusion.include = function (objects, include, options, cb) {
  ...
  function processIncludeItem(objs, include, options, cb) {
    ...
    function includeReferencesMany(callback) {
      ...
      relation.applyScope(null, filter, options, function(err) {
        /**
    ...
    /**
     * Handles hasMany by running one query per included model, which is neccessary for using `limit` and `skip`
     * filter per model.
     */
    function includeHasManySimpleLimitSkip(callback) {
      //Map for Indexing objects by their id for faster retrieval
      var objIdMap2 = includeUtils.buildOneToOneIdentityMapWithOrigKeys(objs, relation.keyFrom);

      var filters = objIdMap2.getKeys().map(function(id) {
        var subFilter = JSON.parse(JSON.stringify(filter));
        subFilter.where[relation.keyTo] = id;
        return subFilter;
      });
      var allTargets = [];

      async.forEach(filters, function(localFilter, cbEach) {
        relation.applyScope(null, localFilter, options, function(err) {
          relation.modelTo.find(localFilter, options, function(err, targets) {
            if (err) {
              return cbEach(err);
            }
        ...
        var targetsIdMap = includeUtils.buildOneToManyIdentityMapWithOrigKeys(allTargets, relation.keyTo);
    ...
    function includeHasManySimple(callback) {
      if (filter.limit !== undefined || filter.skip !== undefined) {
        return includeHasManySimpleLimitSkip(callback);
      }
      //Map for Indexing objects by their id for faster retrieval
      var objIdMap2 = includeUtils.buildOneToOneIdentityMapWithOrigKeys(objs, relation.keyFrom);
    ...
    function includeHasMany(callback) {
      ...
      relation.applyScope(null, filter, options, function() {
